...
public class Ships {
    ...
    //    public Ship(String name, int price, int MLP, int weight, int attack, int speed, int fuelCapacity){
        Ship tankShipOne = new Ship("Titan 1", 1000, 2, 70, 5, 4, 600);
        list.add(tankShipOne);
        
        Ship tankShipTwo = new Ship("Titan 2", 2000, 5, 85, 7, 4, 750);
        list.add(tankShipTwo);

        Ship tankShipThree = new Ship("Titan 3", 5000, 100, 10, 10, 5, 1000);
        list.add(tankShipThree);        
        
        
        
        Ship speedShipOne = new Ship("Banshee 1", 1000, 2, 25, 5, 5, 600); 
        list.add(speedShipOne);

        Ship speedShipTwo = new Ship("Banshee 2", 2000, 5, 15, 5, 7, 750);
        list.add(speedShipTwo);

        Ship speedShipThree = new Ship("Banshee 3", 5000, 10, 5, 5, 10, 1000);
        list.add(speedShipThree);        
        
        
        
        //ships that for people who don't invest in pilot
        Ship normalShipOne = new Ship("Rusty 1", 1000, 0, 50, 3, 4, 500);
        list.add(normalShipOne);
        
        Ship normalShipTwo = new Ship("Rusty 2", 2000, 0, 30, 4, 5, 700);
        list.add(normalShipTwo);

        
        
        Ship tradeShipOne = new Ship("Serenity 1", 1000, 0, 70, 5, 3, 600);
        list.add(tradeShipOne);

        Ship tradeShipTwo = new Ship("Serenity 2", 2000, 0, 75, 5, 2, 700);
